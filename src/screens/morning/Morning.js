...
import { getAllItems, setAllItems, AllItemsError } from "../../redux/action/allItem";
import {getCreateMorning, setCreateMorning, createMorningError} from "../../redux/action/createmorningaccepted";
import NetInfo from "@react-native-community/netinfo";
import MyModal from "../../components/button/Mymodal";
import { checkInternet } from "../../api/internet";
import {setMorningData,MorningDataclear} from "../../redux/action/setmorningdata";
...
class Morning extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            data:[],pointerEvents:false,showmsg:true
        }
    }
    ...
    async dataConnection(){
        ...
        if(this.props.getMorning.data!=null && connection.isConnected ==true && connection.isInternetReachable==true ){
            this.createmorning();
    ...
    onPress() {
        ...
        let data = {
            Mileage, Trailar, generalRemark, currentDate, trcukid, nilStatus,selectedUniqueItems
        }
    ...
    element(cellIndex, data, index) {
        ...
        if (cellIndex == 1) {
            return <TouchableOpacity style={[styles.container]}
                ...
                {data == "0" || data == ""}
                {data == "Y" && <View style={styles.yesbtn}>
                    <Text style={styles.btnText}>Y</Text>
                </View>}

                {data == "N" && <View style={styles.nobtn}>
    ...
    async getMorningAccepted_by_id() {
        let url = URL + 'getMorningAccepted_by_id/'+this.props.createMorning.data?.id
        const requestOptions = {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
        ...
        try {
            ...
            if (responseData.response == 1) {
                ...
                let event = {
                    Mileage :  data.mileage, Trailar:  data.trailerid,
                     generalRemark:data.general, currentDate:data.datetime,
                    ...
                    <Header style={styles.headersty}>
                        ...
                        <Right>
                            <View style={styles.rightbox} >
                                <Text style={styles.text}>{this.props.SelectedTruckDetail.data}</Text>
                            </View>
                        </Right>
                    </Header>
                            ...
                            <View style={styles.content} pointerEvents={this.state.pointerEvents==true?"none":"auto"}>
                                ...
                                <View style={styles.mt20}>
                                    <View style={{ flexDirection: "row" }}>
                                        ...
                                        <View style={{ width: "40%" }}>
                                            <TouchableOpacity style={styles.accptbtn} 
